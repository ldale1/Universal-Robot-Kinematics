...
from portmark import generate_coords, cartons_enum, carton
...
def get_joint_data(df, dfi):
    ...
    flange_vec_a = [-0.125, 0, -0.038, 1]  # altered to tcp
    flange_vec_b = [0, -0.1808, 0, 1]  # altered to tcp
...
if __name__ == "__main__":
    ...
    ax.view_init(elev=-160, azim=-38.5)
    ax.plot(path_x, path_y, path_z)
    ax.plot(print_x, print_y, print_z, 'r.')

    # # Coords stack coords
    # def getpcs(mypcs):
    #     rx = translation_matrix([0, 0, 0, 0], 'x', pi * 272/180)
    #     ry = translation_matrix([0, 0, 0, 0], 'y', pi * -272/180)
    #     rz = translation_matrix([0, 0, 0, 0], 'z', -pi * -(45+272)/180)
    #     #tx = translation_matrix([-0.141, 0.838, 0.085, 1], 'x', 0)
    #     tx = translation_matrix([0, 0, 0, 1], 'x', 0)
    #     xx, yy, zz, one = rx @ ry @ rz @ tx @ np.array(mypcs + [1])
    #     return xx, yy, zz
    #
    # for side in ["A", "B"]:
    #     pcsss = generate_coords(carton, side=side, perfect=True)
    #
    #     kwargs = {"marker": "*", "color": "m" if side == "A" else "y"}
    #     pc_x1 = [ax.scatter(xx, yy, zz, **kwargs) for xx, yy, zz in [getpcs([x, y, z]) for x, _, _, y, z, _ in pcsss]]
    #     pc_x2 = [ax.scatter(xx, yy, zz, **kwargs) for xx, yy, zz in [getpcs([x, y, z]) for _, x, _, y, z, _ in pcsss]]
    #     pc_x3 = [ax.scatter(xx, yy, zz, **kwargs) for xx, yy, zz in [getpcs([x, y, z]) for _, _, x, y, z, _ in pcsss]]
    ...
    def data_gen():
        ...
        while True:
            ...
            if cnt >= len(joint_df):
                break
            yield get_joint_data(joint_df, cnt), get_speed_data(speed_df, cnt)
    ...
    def run(data):
        # update the data
        (joint_x, joint_y, joint_z, isjoint), (vx, vy, vz, wx, wy, wz) = data
        line.set_data(joint_x, joint_y)
        line.set_3d_properties(joint_z)
