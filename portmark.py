...
class UR10_RTDE():
    ...
    def process(self):
        ...
        while True:
            ...
            if self.record and (not (program_counter % 1)):
...
def print_coord_to_tasks(*print_coords: list, starting: int = 1, alternating: bool = True):
    ...
    for print_coord in print_coords_list:
        control_enum = ((starting - 1) % 2) + 1 if alternating else starting
...
def generate_coords(stack_format, side="A", perfect=True):
    ...
    if stack_format.value == 1:
        ...
        cz, cx, cy = 300, 300, 200
        config = chilled_config_func(5, side == "A")
...
if __name__ == "__main__":
    ...
    print_tasks_a = print_coord_to_tasks(print_coords_a)

    print_coords_b = generate_coords(carton, side="B", perfect=True)
    print_tasks_b = print_coord_to_tasks(print_coords_b)
